...
class StyleTransferLosses(VGG19):
    def __init__(self, weight_file, content_img, style_img, content_layers, style_layers,
        ...
        with torch.no_grad():
            for name, layer in self.named_children():
                ...
                if name in content_layers:
                    ...
                    if scale_by_y:
                        self.weights[name] = torch.minimum(content_features, torch.sigmoid(content_features))
    ...
    def forward(self, input):
        ...
        for name, layer in self.named_children():
            ...
            if name in self.content_layers:
                ...
                content_loss += (torch.mean(loss ** 2) * self.content_weights[name])
...
def total_variation_loss(location, curve_s, curve_e, K=10):
    ...
    dist_to_centroid = torch.mean(torch.sum((utils.projection(x_sig_nns) - utils.projection(se_vec)[..., None, :]) ** 2, dim=-1))
...
def tv_loss(x):
    diff_i = torch.mean((x[..., :, 1:] - x[..., :, :-1]) ** 2)
    diff_j = torch.mean((x[..., 1:, :] - x[..., :-1, :]) ** 2)
    loss = diff_i + diff_j
    return loss
